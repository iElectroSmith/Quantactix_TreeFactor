...
public:
    ...
    std::vector<double> m_vec_month_theta ;
    arma::umat* m_pMatx_Xorder ;

    // constructors
    CAPTree( ) : m_vec_month_theta( 1 , 0.0 ) , 
                 m_pMatx_Xorder( 0 ) , 
                 m_numData_inNode( 0 ) , 
                 m_nodeID( 1 ) , 
                 m_varIndex2Split( 0 ) , 
    ...
    CAPTree( size_t dim_theta ) : m_vec_month_theta( dim_theta , 0.0 ) , 
                                  m_pMatx_Xorder( 0 ) , 
                                  m_numData_inNode( 0 ) , 
                                  m_nodeID( 1 ) , 
                                  m_varIndex2Split( 0 ) , 
    ...
    CAPTree( size_t dim_theta , arma::umat* Xordermat ) : m_vec_month_theta( dim_theta , 0.0 ) , 
                                                          m_pMatx_Xorder( Xordermat ) , 
                                                          m_numData_inNode( 0 ) , 
                                                          m_nodeID( 1 ) , 
                                                          m_varIndex2Split( 0 ) , 
                                                          m_valueIndex2Split( 0 ) , 
    ...
    CAPTree( size_t dim_theta ,   //state.num_months
             ...
             size_t ID ,          //1 ,
             APTree_Pt p ,         //0 ,
             arma::umat* Xordermat ) : m_vec_month_theta( dim_theta , 0.0 ) , 
                                       m_pMatx_Xorder( Xordermat ) , 
                                       m_numData_inNode( N ) , 
                                       m_nodeID( ID ) , 
                                       m_varIndex2Split( 0 ) , 
                                       ...
                                       m_leftChild( 0 ) , 
                                       m_rightChild( 0 ) , 
                                       m_iter( 0 ) {   DEBUG_PRINT(""); }


    // functions
    void set_theta( std::vector<double>& theta ) { this->m_vec_month_theta = theta ; }
    ...
    void set_iter( size_t iter ) { this->m_iter = iter ; }

    size_t get_varIndex2Split( ) const { return m_varIndex2Split ; }
    size_t get_treeDepth( ) const { return m_treeDepth ; }
    std::vector<double> get_theta( ) { return m_vec_month_theta ; }
    double get_theta( size_t ind ) const { return m_vec_month_theta[ ind ] ; }
    size_t get_thetaSize( ) const { return m_vec_month_theta.size( ) ; }
    ...
    size_t get_iter( ) const { return m_iter ; }
    ...
    void get_vecOfBtmNodes( vec_APTree_Pt& vecBtmNodes ) ;    // return a vector of bottom nodes

    void get_vecNoGrandChildsNodes( vec_APTree_Pt& vecNoGrandChildsNodes ) ;   // get nog nodes
    void get_AllNodes( vec_APTree_Pt& vecAllNodes) ;                        // get ALL nodes
    void get_AllNodes_const( vec_APTree_cnstPt& vecAllNodesConst ) const ;              // get ALL nodes (const)
    APTree_Pt get_pt2TopNode( ) ;                              // get pointer to the top node (root node) of the tree
    APTree_Pt find_BtmNodeOfData( arma::mat& matx_x , size_t& row_index ) ; // search tree, find bottom node of the data
    ...
    void copyTree( APTree_Pt pNewTree, APTree_cnstPt pOldTree) ; // copy tree from o to n
    void copy_only_root( APTree_Pt pOldTree) ;  // copy tree, point new root to old structure
    friend std::istream& operator>>( std::istream& , CAPTree& ) ;

    void split_Xorder( arma::umat& matx_Xorder_left ,
                       arma::umat& matx_Xorder_right ,
                       arma::umat& matx_Xorder ,
                       size_t split_point , 
                       size_t split_var , 
                       CState& state ) ;
    void predict( arma::mat matx_X , 
                  arma::vec vec_months , 
                  arma::vec& vec_output ) ;

    void grow( bool& b_breakFlag , 
               CAPTreeModel& model , 
               CState& state , 
               size_t& iter , 
               std::vector<double>& vec_criterion_values ) ;

    void grow_APTree_TS( bool& b_breakFlag , CAPTreeModel& model , CState& state ) ;
...
private:
    ...
    size_t m_iter ;    // which iteration this node splits
