...
public:
    // define types
    typedef CAPTree* APTree_p;
    typedef const CAPTree* APTree_cp;
    ...
    // constructors
    CAPTree( ) : theta( 1 , 0.0 ) , Xorder( 0 ) , N( 0 ) , ID( 1 ) , v( 0 ) , c_index( 0 ) , 
                c( 0.0 ) , depth( 0 ) , parentNode( 0 ) , leftChild( 0 ) , rightChild( 0 ) , iter( 0 ) { }

    CAPTree( size_t dim_theta ) : theta( dim_theta , 0.0 ) , Xorder( 0 ) , N( 0 ) , ID( 1 ) , v( 0 ) , 
                                 c_index( 0 ) , c( 0.0 ) , depth( 0 ) , parentNode( 0 ) , 
                                 leftChild( 0 ) , rightChild( 0 ) , iter( 0 ) { }
    
    CAPTree( size_t dim_theta , arma::umat* Xordermat ) : theta( dim_theta , 0.0 ) , Xorder( Xordermat ) , 
                                                         N( 0 ) , ID( 1 ) , v( 0 ) , c_index( 0 ) , c( 0.0 ) , 
                                                         depth( 0 ) , parentNode( 0 ) , leftChild( 0 ) , 
                                                         rightChild( 0 ) , iter( 0 ) { }
    CAPTree( size_t dim_theta , size_t depth , 
            size_t N , size_t ID , 
            APTree_p p , arma::umat* Xordermat ) : theta( dim_theta , 0.0 ) , Xorder( Xordermat ) , 
                                                  N( N ) , ID( ID ) , v( 0 ) , c_index( 0 ) , 
                                                  c( 0.0 ) , depth( depth ) , parentNode( p ) , 
                                                  leftChild( 0 ) , rightChild( 0 ) , iter( 0 ) { }
    ...
    void setl( APTree_p l ) { this->leftChild = l; }
    void setr( APTree_p r ) { this->rightChild = r; }
    void setp( APTree_p p ) { this->parentNode = p; }
    ...
    APTree_p getp( ) { return parentNode; }
    APTree_p getr( ) { return rightChild; }
    APTree_p getl( ) { return leftChild; }
    ...
    APTree_p getParent( size_t nid );                  // get node pointer from node ID, 0 if not there;
    ...
    bool isnog( );
    void copyTree( APTree_p n , APTree_cp o ); // copy tree from o to n
    void copy_only_root( APTree_p o );  // copy tree, point new root to old structure
    friend std::istream& operator>>( std::istream& , CAPTree& );

    void split_Xorder( arma::umat& Xorder_left , arma::umat& Xorder_right , arma::umat& Xorder , 
                       size_t split_point , size_t split_var , State& state );
    void predict( arma::mat X , arma::vec months , arma::vec& output );

    void grow( bool& break_flag , CAPTreeModel& model , State& state , size_t& iter , 
...
private:
    ...
    APTree_p parentNode; // pointer to the parent node
    APTree_p leftChild; // pointer to left child
    APTree_p rightChild; // pointer to right child
